...
// - readOnly = true의 효과
// "이 트랜잭션은 데이터를 변경하지 않는다"라고 JPA에게 알림, 이 트랜잭션에서는 조회만 하겠다. -> 최적화
// 하이버네이트가 변경 감지를 위한 스냅샷을 만들지 않아 메모리/성능에 유리
// Insert/Update/Delete가 필요한 메서드는 @Transactional을 다시 붙인다. (디폴트 값 false?)
