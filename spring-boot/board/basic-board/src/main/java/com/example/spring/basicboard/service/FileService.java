...
public class FileService {
    ...
    // 메모리 : getBytes()는 파일 "전체"를 byte[]로 힙 메모리에 올린다. -> 큰 파일/동시 업로드 시 OutOfMemory 위험
    // 반면 transferTo는 통째로 올리지 않고 옮기며, 같은 디스크면 복사가 아니라 이동 방식이라 가볍고 빠르다.
    ...
    public Resource downloadFile(String fileName) {
        ...
        try {
            ...
            // 파일을 가리키는 Resource 생성 (실제로 읽어들이는 것이 아니라 위치만 잡아두는 단계)
            ...
            throw new IllegalStateException("파일 경로가 잘못되었습니다 : " + fileName, e);
        } catch (IOException e) {
